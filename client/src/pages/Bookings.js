...
function Bookings() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (bus, record) => (
        <div>
          <b>{bus?.name} ({bus?.number})</b>
          <div style={{ fontSize: 12, color: "#1e40af", fontWeight: 600 }}>
            {record.boardingStop || bus?.from} ➔ {record.alightingStop || bus?.to}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      dataIndex: "seats",
      key: "seats",
      render: (seats) => <Tag color="geekblue">{seats.join(", ")}</Tag>,
    },
    {
      title: "Hành khách",
    ...
    },
    {
      ...
      render: (status) => {
        if (status === "Boarded") {
          return <Tag color="cyan">Đã lên xe</Tag>;
    ...
    },
    {
      ...
      render: (text, record) => (
        <div className="d-flex gap-2">
          ...
          >
            <i className="ri-qr-code-line mr-1"></i> Xem vé & QR
          </Button>

          {record.status === "Paid" && (
            <Popconfirm
              title="Bạn có chắc chắn muốn hủy vé và yêu cầu hoàn tiền không?"
              okText="Đồng ý hủy"
              cancelText="Không"
